-- Rollover hour for the daytrip planner: the local hour after which it stops
-- suggesting a trip for today (too late to start one) and plans for tomorrow
-- instead — the pane title switches to "Planning a daytrip tomorrow?" and shows
-- tomorrow's windows. Default 18:00; tune from /admin (e.g. as early as 15:00).

alter table public.safety_rules
  add column if not exists daytrip_rollover_hour int not null default 18;

alter table public.safety_rules
  add constraint safety_rules_daytrip_rollover_hour_check
    check (daytrip_rollover_hour between 0 and 23);
